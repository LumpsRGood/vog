...
function setField(fields: Record<string, unknown>, map: Map<string, string>, displayName: string, value: unknown) { const internal = map.get(displayName.trim().toLowerCase()); if (internal && value !== null && value !== undefined && value !== "") fields[internal] = value; }
Deno.serve(async (request) => {
  ...
  try {
    ...
    const cols = await graphFetch(token, `/sites/${site.id}/lists/${activityList.id}/columns?$select=name,displayName`); const map = new Map<string, string>(cols.value.map((c: { name: string; displayName: string }) => [c.displayName.trim().toLowerCase(), c.name]));
    const a = payload.record; const fields: Record<string, unknown> = {};
    setField(fields, map, "Activity ID", text(a.id) || crypto.randomUUID()); setField(fields, map, "Case ID", text(a.case_id)); setField(fields, map, "Performed By", text(a.staff_member)); setField(fields, map, "Guest Contacted", text(a.guest_contacted)); setField(fields, map, "Activity Notes", text(a.note)); setField(fields, map, "Activity Date", a.activity_date || new Date().toISOString());
